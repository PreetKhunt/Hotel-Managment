    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500">Manage partner shops and exclusive hotel guest offers.</p>
        </div>
        <div className="flex gap-2">
          <Button 
            variant={activeTab === 'shops' ? 'default' : 'outline'} 
            onClick={() => { setActiveTab('shops'); setIsAdding(false); }}
            className={activeTab === 'shops' ? 'bg-violet-600 hover:bg-violet-700' : ''}
          >
          ...
          <Button 
            ...
            className={activeTab === 'offers' ? 'bg-violet-600 hover:bg-violet-700' : ''}
          >
          ...
          <Button 
            ...
            className={activeTab === 'redeem' ? 'bg-violet-600 hover:bg-violet-700' : ''}
          >
      ...
      {activeTab !== 'redeem' && (
        <div className="flex justify-end mb-4">
          <Button onClick={() => setIsAdding(!isAdding)} className="bg-slate-800 hover:bg-slate-900">
      ...
      {isAdding && activeTab === 'shops' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input placeholder="Shop Name *" value={newShop.shop_name} onChange={e => setNewShop({...newShop, shop_name: e.target.value})} />
          <Input placeholder="Brand Name" value={newShop.brand_name} onChange={e => setNewShop({...newShop, brand_name: e.target.value})} />
          <Input placeholder="Category *" value={newShop.category} onChange={e => setNewShop({...newShop, category: e.target.value})} />
          <Input placeholder="Address *" value={newShop.address} onChange={e => setNewShop({...newShop, address: e.target.value})} />
          <div className="md:col-span-2 flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setIsAdding(false)}>Cancel</Button>
            <Button onClick={handleCreateShop} disabled={createShop.isPending} className="bg-violet-600 hover:bg-violet-700">
      ...
      {isAdding && activeTab === 'offers' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input placeholder="Offer Title *" value={newOffer.title} onChange={e => setNewOffer({...newOffer, title: e.target.value})} />
          <Input placeholder="Description *" value={newOffer.description} onChange={e => setNewOffer({...newOffer, description: e.target.value})} className="md:col-span-2" />
          <Input placeholder="Offer Type (e.g., Flat Discount, Percentage) *" value={newOffer.offer_type} onChange={e => setNewOffer({...newOffer, offer_type: e.target.value})} />
          <Input type="number" placeholder="Discount Value *" value={newOffer.discount_value || ''} onChange={e => setNewOffer({...newOffer, discount_value: Number(e.target.value)})} />
          <Input type="date" placeholder="Expiry Date *" value={newOffer.expiry_date} onChange={e => setNewOffer({...newOffer, expiry_date: e.target.value})} />
          <Input type="number" placeholder="Max Redemptions (0 for unlimited)" value={newOffer.max_redemptions || ''} onChange={e => setNewOffer({...newOffer, max_redemptions: Number(e.target.value)})} />
          <div className="md:col-span-2 flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setIsAdding(false)}>Cancel</Button>
            <Button onClick={handleCreateOffer} disabled={createOffer.isPending} className="bg-violet-600 hover:bg-violet-700">
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {activeTab === 'shops' && (
                loadingShops ? (
                  ...
                  <tr key={shop.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4 font-medium text-slate-800">{shop.shop_name} {shop.brand_name && <span className="text-xs text-slate-400 block">{shop.brand_name}</span>}</td>
                    <td className="p-4"><span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs">{shop.category}</span></td>
                    <td className="p-4 text-slate-500">{shop.address}</td>
                    <td className="p-4 text-right flex justify-end gap-2">
                      <Button variant="ghost" size="icon" onClick={() => handleDeleteShop(shop.id)} className="h-8 w-8 text-slate-400 hover:text-red-600">
              ...
              {activeTab === 'offers' && (
                loadingOffers ? (
                  ...
                  <tr key={offer.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4 font-medium text-slate-800">{offer.title}</td>
                    <td className="p-4 text-slate-600">{offer.shop_name || offer.brand_name}</td>
                    <td className="p-4 text-slate-500">{new Date(offer.expiry_date).toLocaleDateString()}</td>
                    <td className="p-4 text-slate-500">{offer.current_redemptions} / {offer.max_redemptions || '∞'}</td>
                    <td className="p-4 text-right flex justify-end gap-2">
                      <Button variant="ghost" size="icon" onClick={() => handleDeleteOffer(offer.id)} className="h-8 w-8 text-slate-400 hover:text-red-600">
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    </td>
                  </tr>
              ...
              {activeTab === 'shops' && shops?.data.length === 0 && !loadingShops && (
                <tr><td colSpan={4} className="p-8 text-center text-slate-500">No partner shops found.</td></tr>
              )}
      ...
      {activeTab === 'redeem' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm max-w-lg mx-auto mt-8">
          <h2 className="text-xl font-semibold mb-4 text-slate-800">Redeem Coupon</h2>
          <p className="text-sm text-slate-500 mb-6">Enter the coupon code provided by the guest to validate and redeem it.</p>
          <div className="flex gap-2">
            <Input 
